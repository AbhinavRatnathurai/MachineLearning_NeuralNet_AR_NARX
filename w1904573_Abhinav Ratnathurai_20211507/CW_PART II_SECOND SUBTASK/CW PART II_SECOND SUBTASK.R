...
# 18th and 19th HOUR
data$Hour_18 <- data$`0.75`
data$Hour_19 <- data$`0.79166666666666663`


#Additional input vectors for NARX configuration
data$Lag_18 <- c(rep(NA, 18), data$`0.83333333333333337`[1:(nrow(data) - 18)])
data$Lag_19 <- c(rep(NA, 19), data$`0.83333333333333337`[1:(nrow(data) - 19)])

#Update the create_io_matrix function to include Lag_18 and Lag_19
create_io_matrix_narx <- function(data, max_lag = 4, include_lag_7 = TRUE, include_lag_18_19 = TRUE) {
  n <- nrow(data)
  input_columns <- c()
  ...
  if (include_lag_18_19) {
    data$Lag_18 <- c(rep(NA, 18), data$`0.83333333333333337`[1:(n - 18)])
    data$Lag_19 <- c(rep(NA, 19), data$`0.83333333333333337`[1:(n - 19)])
    input_columns <- c(input_columns, "Lag_18", "Lag_19")
  }
...
best_one_hidden_layer_idx <- which.min(comparison_table_narx_2$RMSE[comparison_table_narx_2$Description %in% c("5", "10", "15")])
best_two_hidden_layer_idx <- which.min(comparison_table_narx_2$RMSE[comparison_table_narx_2$Description %in% c("5-5", "10-5", "5-10", "10-10", "15-15")])
...
cat("Best one-hidden layer network has", comparison_table_narx_2$Weight_Parameters[best_one_hidden_layer_idx], "weight parameters.\n")
cat("Best two-hidden layer network has", comparison_table_narx_2$Weight_Parameters[best_two_hidden_layer_idx], "weight parameters.\n")


#Find the best model based on RMSE and MAPE
best_model_idx_rmse_narx <- which.min(comparison_table_narx_2$RMSE)
best_model_idx_mape_narx <- which.min(comparison_table_narx_2$MAPE)
